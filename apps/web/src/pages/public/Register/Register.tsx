...
import type { RegisterTenantData, RegisterResponse } from "../../../services/api";
...
export default function Register() {
  ...
  useEffect(() => {
    ...
    if (params.get("payment") === "success") {
      ...
      setMessage({ text: "Pagamento confirmado! Agora finalize o cadastro.", type: "success" });
    }
  }, [location]);
 ...
 const handlePayment = () => {
  // Simula pagamento concluído sem redirecionar
  setPaymentCompleted(true);
  setMessage({ text: "Pagamento confirmado! Agora finalize o cadastro.", type: "success" });
};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!paymentCompleted) {
      setMessage({ text: "Efetue o pagamento antes de cadastrar.", type: "error" });
      return;
    }
    setLoading(true);
    setMessage(null);
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <button
            ...
            style={{
              ...
              transition: "all 0.2s",
              marginTop: "8px",
              opacity: loading || !paymentCompleted ? 0.6 : 1,
              cursor: loading || !paymentCompleted ? "not-allowed" : "pointer",
              boxShadow: loading || !paymentCompleted ? "none" : "0 8px 20px rgba(0, 229, 255, 0.3)",
            }}
            onMouseEnter={(e) => {
